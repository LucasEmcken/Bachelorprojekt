...
def plot_error(model_name = "OPT_NMF",data_name = "art", lr = "0.1", start_index = 1, end=10):
	data = np.load("./losses/"+str(data_name)+"_"+str(model_name)+"_"+str(lr)+"_"+str(start_index)+"_"+"lr_test.npy")
	data = np.expand_dims(data,0)
	
	for nr_components in range(start_index+1,end+1):
...
plot_error(model_name="GRAD_NMF", data_name="art", lr="0.01",end=8)
plot_error(model_name="GRAD_NMF", data_name="art", lr="1",end=6)
plot_error(model_name="GRAD_NMF", data_name="art", lr="0.1")

# plot_error(model_name="OPT_NMF", data_name="art", lr="0.1")
# plot_error(model_name="OPT_NMF", data_name="art", lr="0.01")
# plot_error(model_name="OPT_NMF", data_name="art", lr="1")
