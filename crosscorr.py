...
if __name__ == '__main__':
    ...
    for k in components_range:
        print('fitting')
        model = ShiftNMF(X, k, lr=0.2, alpha=1e-6, patience=1000, min_imp=0)
        W_est,H_est,tau_est, running_loss_hybrid = model.fit(verbose=True, return_loss=True, max_iter=1000, tau_iter=0, Lambda=0.0)
        ...
        # get the max cross-correlation for each component
        # C = np.zeros((H.shape[0], H_est.shape[0]))
        # C_ind = np.zeros((H.shape[0], H_est.shape[0]))
        
        # for i in range(H.shape[0]):
        #     for j in range(H_est.shape[0]):
        #         #compute cross-correlation
        #         # cross = correlate(H[i], H[j], mode='same')
        #         lags, cross = normalized_cross_correlation(H[i], H_est[j], 1000)
        #         C[i,j] = np.max(cross)
        #         C_ind[i,j] = lags[np.argmax(cross)]
        ...
        #get the max cross-correlation for each component
        # stds.append(np.std(np.max(C, axis=1)))
        stds.append(0)
        autocorrelations.append(np.max(C))
        
        # autocorrelations.append(np.max(C))
        
        # temp_autocorr = []
        # for i in range(H.shape[0]):
        #     corr, _ = pearsonr(H[:,i], H_est[:,i])
        #     temp_autocorr.append(corr)
        
        # stds.append(np.std(temp_autocorr))
        # autocorrelations.append(np.mean(temp_autocorr))
